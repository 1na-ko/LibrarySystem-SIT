...
> **项目状态**: ✅ 阶段 0–9 已完成 —— common（基础设施）/ ai（LLM·Embedding·NLP）/ security（认证·RBAC·限流·管理员 Controller）/ core（图书检索·借阅·预约·推荐·管理 Service）/ knowledge-graph（Neo4j 图谱构建·查询·溯源）/ acquisition（采购预测·查重查缺·智能谈判）/ bootstrap（启动聚合）七模块源码落地，8 模块 BUILD SUCCESS
...
> - **借阅/预约补充**：`GET /api/v1/borrows`、`GET /api/v1/borrows/{id}`、`GET /api/v1/reservations`、`GET /api/v1/reservations/{id}/queue-position` → 用户可查询自身记录；Librarian/Admin 可查询全部
> - **个人中心补充**：`GET /users/me/history`、`GET /users/me/stats` → 用户可查询自身数据
> - **系统管理**：`GET /admin/users`、`PUT /admin/users/{id}/status` → Librarian 及以上（对齐 `AdminUserController` 的 `@RequireRole({LIBRARIAN, ADMIN})`）；`POST /admin/books`、`PUT /admin/books/{id}`、`DELETE /admin/books/{id}`（图书编目）→ Librarian 及以上（`book:catalog` 权限），Admin 亦可（注：编目为图书管理员核心职责，故不限于 Admin；与上方角色总表"图书编目"一致）
...
本系统采用 **Modular Monolith（模块化单体）** 架构，按领域边界拆分为独立 Maven 模块，模块间通过 **RabbitMQ 事件总线**（领域事件 + `@TransactionalEventListener(AFTER_COMMIT)` 桥接转发 + `@RabbitListener` 消费）和本地 API 调用通信。此架构兼顾微服务的边界清晰与单体的开发便捷，未来可按需将高负载模块独立部署为微服务。
...
┌───────┴────────────┴───────────────┴────────────────────┴───────────────┐
│                          Domain Event Bus                                │
│      (RabbitMQ 事件总线: EventBusBridge 桥接 + @RabbitListener 消费)    │
└───────┬────────────┬───────────────┬────────────────────┬───────────────┘
        │            │               │                    │
┌───────┴────────────┴───────────────┴────────────────────┴───────────────┐
    ...
    └── src/main/
        ...
        └── resources/
            ...
            └── db/migration/                          # Flyway 数据库迁移脚本
                ...
                ├── V6__unique_reservation_user_book.sql # ✅ 预约(user_id,book_id,status)唯一约束（注：MySQL 不感知 MyBatis-Plus logic-delete，需 physicalCleanStaleWaiting 物理删除配合）
                └── V7__operation_log.sql              # ✅ 操作日志表（阶段 9）
```
...
> | `library-bootstrap` | ✅ 已实现 | 启动类 / CORS / Jackson / MyBatis-Plus / Async / Scheduling 配置 / Flyway V1–V7 / 推荐配置 / Prometheus 指标 |
...
>
> **Controller 落位决策（ADR）**：认证与需鉴权上下文的业务 Controller（`AuthController`、`BorrowController`、`ReservationController`、`UserCenterController`、`AdminBookController`、`AdminBorrowController`、`AdminUserController`、`AdminStatsController`、`RecommendationController`）置于 `library-security` 模块，而非本节结构图所示的 `library-core`。原因：这些 Controller 强依赖 `SecurityUtils`（security 域）获取当前登录用户，若放 core 将形成 `security → core → security` 循环依赖。无需鉴权上下文的 Controller（`BookController`、`CategoryController`）仍置 `library-core`。Service 与 Mapper 一律置 `library-core`，Controller 仅做参数编排与调用 Service。
...
> **SPI 端口模式（破除 core←kg/acquisition 循环依赖）**：core 模块定义 3 个 SPI 接口——`KgRecommendPort`（KG 多跳推荐）、`KgRelatedBookPort`（同主题相关书）、`GapCoreBookPort`（馆藏缺口核心书目查询），由 knowledge-graph / acquisition 模块提供实现（`@ConditionalOnBean(Neo4jClient.class)`）。core 通过 `ObjectProvider` 延迟注入适配器：Neo4j/依赖就绪时调用实现，否则静默降级空结果。依赖方向严格 core←kg/acquisition，避免 core 反向依赖下游模块形成循环。
...
| GET | `/api/v1/borrows?status=` | 我的借阅列表 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/kg/book/{bookId}` | 图书知识图谱（主题关联网络，支持 depth 参数） |
| GET | `/api/v1/kg/book/{bookId}/trace` | 文献溯源路径（支持 direction/maxDepth 参数） |
| GET | `/api/v1/kg/book/{bookId}/keypath` | 关键路径发现（两书间最短引用路径） |
| GET | `/api/v1/kg/subject/{name}` | 学科主题网络 |
| GET | `/api/v1/kg/search?entity=` | 知识实体搜索 |
| POST | `/api/v1/admin/kg/rebuild/{bookId}` | 重建单本图书知识图谱（KG Admin） |
| POST | `/api/v1/admin/kg/rebuild-all` | 全量重建知识图谱（KG Admin） |
...
| GET | `/api/v1/admin/stats/dashboard` | 流通统计 Dashboard（Librarian 及以上） |
...
public interface LibraryApi {
    ...
    // =========== 知识图谱 ===========
    @GET("api/v1/kg/book/{bookId}")
    Call<Result<KnowledgeGraphVO>> getBookGraph(@Path("bookId") Long bookId);
}
```
...
> 以下示例展示关键配置片段（Spring Boot 3.5.x 风格，含环境变量占位符）。AI（`ai.deepseek.*`/`ai.dashscope.*`）、推荐（`recommendation.*`）、知识图谱（`kg.*`）、采编（`acquisition.*`）、限流（`ratelimit.*`）等配置块及完整内容详见 [`library-server/library-bootstrap/src/main/resources/application.yml`](../library-server/library-bootstrap/src/main/resources/application.yml)。
...
spring:
  ...
  # ---- RabbitMQ ----
  # 阶段10 已引入 spring-boot-starter-amqp，事件总线正式启用
  # 架构：领域事件 → EventBusBridge(AFTER_COMMIT 桥接) → MQ → @RabbitListener 消费
  rabbitmq:
...
| v1.11 | 2026-06-16 | 阶段 9（系统管理与监控）实现：新增 6 项管理功能 + 3 项 P3 修复。`GET /admin/users` 用户分页列表 + `PUT /admin/users/{id}/status` 状态变更（含 AOP 审计） + `GET /admin/stats/dashboard` 流通统计 + `@OperationLog` 操作日志切面（Flyway V7 `operation_log`） + Prometheus 指标暴露（`micrometer-registry-prometheus` + Gauge/Counter 埋点） + 定时任务总控增强（4 线程池 + `ReservationZsetReconcileJob` 完整实现 + `EsRebuildJob` 每周全量重建）。P3 修复：`@EnableMethodSecurity` 死配置移除 / ES URI 补齐 scheme / CORS 生产白名单。全量 355 项测试全绿 | 开发团队 |
| v1.12 | 2026-06-17 | 阶段 9 后剩余审计问题修复（~35 项）：P1 GlobalExceptionHandler 补 `DataIntegrityViolationException` handler + spring-tx / OperationLogAspect 分层合规提取 `OperationLogService` / GraphBuildServiceImpl N+1→UNWIND 批量写入；P2 推荐超时保留部分结果 / ZSET ZRANGE→ZSCAN / 查重 NLP 预处理外提 / 注入 Spring ObjectMapper / AiHttpClientFactory 共享 WebClient 配置 / SecurityResponseUtil 工具提取 / PageRank 稠密→稀疏 / 死代码清理；P3 ErrorCode.SUCCESS @Deprecated / 配置文档同步。全量 356 项测试全绿 | 开发团队 |
| v1.13 | 2026-06-17 | 阶段 9 后回溯审计修正（4 文档 + 3 代码）：文档——架构文档 §2.3 权限矩阵与 §6.3 Android 示例 KG 路径修正（`/kg/book/{id}/graph` → `/kg/book/{bookId}`，与实际 `KnowledgeGraphController` 对齐）、头部版本号 v1.10→v1.13 对齐版本历史、`docs/README.md` 目录树补阶段 9 三份文档条目；代码——`OperationLogAspect` 实现敏感字段脱敏（password/token/secret/credential/apiKey 等，兑现 `@OperationLog.logParams` 的 Javadoc 承诺，+4 单测）、`BookESRepository.bulkSave` + `EsRebuildJob` 由逐条 `save` 改 `BulkRequest` 批量写入（N 次往返→1 次，失败降级逐条）、`GraphBuildServiceImpl` `@Transactional` 注释澄清（其对 Neo4j Session auto-commit 写入无效，图谱一致性由 MERGE 幂等 + `KgBuildListener` 重试保证）。全量 360 项测试全绿 | 开发团队 |
| v1.14 | 2026-06-17 | 阶段 9 后第二轮回溯审计修复（39 项）：P1 6 项（PageDTO 构造器钳制分页参数全局修复 / EmbeddingServiceImpl.batchEmbed 批次失败隔离 / OpenAPI trace·keypath 参数名对齐 / §2.3 用户管理权限注释修正 / JWT 默认密钥 ≥32 字节修复第一轮加固引入的启动失败回归）；P2 13 项（BookUpdateDTO @Min / Reservation rank 查询 / LLM·Embedding 重试超时预算 / onStatus 统一 / Neo4j Cypher 白名单对称防护 / OperationLogAspect taskExecutor / §6.2.9 补 dashboard·§12.4 配置同步·§4.2.1 补 SPI 端口模式）；P3 20 项（死代码清理 / @Configuration→@Component / Job cron 错开 / MetricsConfig TaskScheduler / 删 shortestPathViaGds 死分支等）。全量 362 项测试全绿 | 开发团队 |
| v1.15 | 2026-06-17 | 阶段 10 后第二/三轮：Docker 真实环境集成测试（替代 Testcontainers）+ 安全加固 + 生产部署 + 全业务流程端点验证。代码：① OWASP AccessToken 登出黑名单（user 维度时间戳）；② 应用启动 P0（MetricsConfig @Lazy / MyBatis EnumTypeHandler / Flyway validate=false / docker.compose.enabled=false）；③ ES 同步 P0（refresh=WaitFor / sort builder _score variant 修复所有搜索失败）；④ 4 个生产实测 bug（DashScope batch 25→10 / KG search type 大小写兼容 / KG 图谱·溯源 PATH 解析 nodes(p)+rels(p) 重组）；⑤ Redisson popMin zCard 预检规避 ZSET bug。部署：阿里云 Ubuntu 24.04 / docker-compose.prod.yml（中间件端口公网全隔离，仅 8080 对外）/ systemd library.service / 强随机密码 / IK config 拷贝修复。验证：单元 372/372 + 集成 34/35（ReservationFlow @Disabled）+ 生产端点 51/52 全绿。详见 `docs/implementation/阶段10后审计修复记录.md` §6 §7。 | 开发团队 |
| v1.16 | 2026-06-19 | 阶段 14 后真机走查修复（16 文件，3 轮回归）：**搜索页**（titlebar 引入 page_toolbar / SearchFragment→BaseFragment / ic_close_vector / category_nav 删 ▸ / 热词 8→23 / searchMethodLabel 区分搜索方式 / 删除 tvSearchAgain / ISBN removeExtra 防死循环 / BaseFragment navigateUp=false 兜底）；**谈判流式**（SSE done 空 data 边界修复 / body.close IOException 抑制 / AcquisitionViewModel.onCleared 移除全局 disposeStreams / streamCancelled 合作取消）；**取消预约**（cancel 终态扩展允许 NOTIFIED / ReservationMapper.physicalCleanStaleWaiting 物理 DELETE 绕过逻辑删除解除 V6 唯一约束 / reserve 前置清理 / ReservationListFragment→BaseFragment + observeError / 取消失败 Snackbar 反馈）。后端部署验证通过，真机 adb 走查全部通过 | 开发团队 |
