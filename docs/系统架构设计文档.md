...
| **时序预测** | Apache Commons Math | 3.6.x | 轻量 ARIMA 时序预测（替代 Spark MLlib，无需分布式计算） |
...
> **设计决策**：采用模块化单体架构，不单独设立 API 网关模块。路由转发与认证鉴权由 `library-server` 中的 Spring Security 过滤器链统一处理，生产环境在 Nginx 反向代理层实现限流与路由。若未来演进为微服务架构，可将 `library-server` 拆分并引入 Spring Cloud Gateway。
...
│       ├── utils/                       # 工具类
│       ├── annotation/                  # 自定义注解
│       └── llm/                         # LLM 服务封装（DeepSeek + 百炼 Embedding，含降级策略）
├── library-core/                        # 核心业务模块
...
│       ├── model/                       # 图节点/关系模型
│       ├── nlp/                         # NLP 处理（HanLP 分词 + LLM NER/RE 调用）
│       └── config/                      # Neo4j 配置
├── library-acquisition/                 # 智能采编模块
│   └── src/main/java/com/library/acquisition/
│       ├── controller/                  # 采编 API
│       ├── service/                     # 预测/查重/谈判服务
│       ├── ml/                          # 时序预测（ARIMA，Apache Commons Math）
│       ├── repository/                  # 采编数据访问
│       └── config/                      # 模块配置
...
└── library-server/                      # 启动模块（聚合入口）
    └── src/main/java/com/library/
        ├── LibraryApplication.java      # Spring Boot 启动类
        └── config/                      # 全局配置（CORS、拦截器等）
```
...
CREATE TABLE fine_record (
    ...
    paid TINYINT(1) DEFAULT 0 COMMENT '是否已缴（0=未缴, 1=已缴）',
...
| 借阅数量更新 | BorrowService 归还/借出后 | Spring Event → 异步监听器 | MySQL → ES（更新 `borrow_count`、`avail_copies`） |
...
> **设计决策**：图谱构建层的实体识别（NER）和关系抽取（RE）采用 DeepSeek API 替代传统 HanLP 方案。HanLP 仅保留分词和关键词提取等本地轻量处理，NER/RE 等语言理解任务交由 LLM 处理，以获得更高的识别准确率和更强的泛化能力。LLM 调用封装在 `common/llm/LlmService` 中，含降级策略（API 不可用时回退到 HanLP 关键词匹配）。
...
> **设计决策**：采购需求预测采用轻量 ARIMA（Apache Commons Math）替代 Spark MLlib，避免为课设量级数据引入分布式计算框架。谈判策略引擎采用 DeepSeek API 替代 Drools 规则引擎——LLM 能根据供应商画像和历史成交数据生成更灵活、更有针对性的谈判建议，远超硬编码规则的表达能力。所有 LLM 调用通过 `common/llm/LlmService` 统一封装，含降级策略和本地缓存。
...
**预测模型**：基于 **时间序列分析 (ARIMA)**，使用 Apache Commons Math 轻量实现，综合考虑以下特征：
...
public class ProcurementPredictionService {
    ...
    /**
     * 采购需求预测
     * 使用 Apache Commons Math 的 ARIMA 模型（轻量级，单机计算）
     *
    ...
    public List<PurchasePredictionVO> predict(Long subjectId, int months) {
        ...
        // 2. 拟合 ARIMA 模型（Apache Commons Math）
        ARIMA arima = ARIMA.fit(timeSeries, 1, 1, 1); // (p,d,q) 可调
        double[] forecast = arima.forecast(months);
...
> **选型理由**：课设项目数据量在 MB 级别，Apache Commons Math 的 ARIMA 实现单机即可完成。未来若数据量达到 TB 级，可平滑迁移至分布式计算框架，接口层无需改动。
        List<MonthlyStat> historyStats = borrowRecordMapper
            .getMonthlyStatsBySubject(subjectId, 12);
...
public class NegotiationAdvisor {
    ...
    private final LlmService llmService;    // 统一 LLM 封装（common/llm）
...
| v1.2 | 2026-06-15 | 架构升级：智能层改用混合架构——Apache Spark MLlib → Apache Commons Math 轻量 ARIMA；Drools 规则引擎 → DeepSeek API（谈判策略 + NER/RE + 推荐理由）；新增阿里云百炼 Embedding（语义检索增强）；新增 LlmService 统一封装（含降级策略） | 开发团队 |
