...
> **项目状态**: ✅ 初始化框架已搭建，Maven 多模块编译通过（7/7），Android 项目骨架就绪
...
> - **系统管理**：`GET /admin/users`、`PUT /admin/users/{id}/status`、`POST /admin/books`、`PUT /admin/books/{id}`、`DELETE /admin/books/{id}` → Admin 专有
...
└── db/
    └── migration/                      # Flyway 数据库迁移脚本
        ├── V1__init_schema.sql         # ✅ 已创建（基线）
        ├── V2__insert_categories.sql   # 📋 计划中
        └── V3__add_fulltext_index.sql  # 📋 计划中
```

### 12.4 Spring 主配置文件示例

> 以下示例反映当前代码库实际配置（Spring Boot 3.5.x 风格，含环境变量占位符）。详见 [`library-server/library-bootstrap/src/main/resources/application.yml`](../library-server/library-bootstrap/src/main/resources/application.yml)。
...
spring:
  ...
  # ---- 数据源 ----
  datasource:
    url: jdbc:mysql://${DB_HOST:localhost}:${DB_PORT:3306}/${DB_NAME:library_db}?useUnicode=true&characterEncoding=utf8mb4&serverTimezone=Asia/Shanghai
  ...
  # ---- Elasticsearch（Spring Boot 3.x 新配置键） ----
  elasticsearch:
    uris: ${ES_HOST:localhost}:${ES_PORT:9200}

  # ---- Neo4j（Spring Boot 3.x 新配置键） ----
  neo4j:
