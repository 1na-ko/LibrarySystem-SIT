...
> **项目状态**: ✅ 阶段 0–5 已完成 —— common（基础设施）/ ai（LLM·Embedding·NLP）/ security（认证·RBAC·限流）/ core（图书检索·借阅·预约）/ bootstrap（启动聚合）五模块源码落地，8 模块 BUILD SUCCESS，全量 313 项测试全绿；knowledge-graph、acquisition 两模块源码待阶段 7–8 按本蓝本逐步实现
...
│   └── src/main/java/com/library/common/     # 📋 源码待实现（仅 pom 就位）
│       ├── exception/                        # 全局异常定义
...
│   └── src/main/java/com/library/ai/         # 📋 源码待实现
...
├── library-core/                             # 核心业务模块
│   └── src/main/java/com/library/core/       # 📋 源码待实现
│       ├── controller/                       # REST 控制器
│       ├── service/                          # 业务逻辑层
│       ├── mapper/                           # MyBatis 映射器
│       ├── entity/                           # 数据库实体
...
│   └── src/main/java/com/library/kg/         # 📋 源码待实现
│       ├── controller/                       # 知识图谱 API
│       ├── service/                          # 图谱构建/查询服务
...
│   └── src/main/java/com/library/acquisition/ # 📋 源码待实现
│       ├── controller/                       # 采编 API
│       ├── service/                          # 预测/查重/谈判服务
...
│   └── src/main/java/com/library/security/   # 📋 源码待实现
│       ├── filter/                           # JWT 过滤器
│       ├── handler/                          # 认证/授权处理器
    ...
    └── src/main/
        ...
        └── resources/
            ...
            └── db/migration/                 # Flyway 数据库迁移脚本
                ├── V1__init_schema.sql       # ✅ 基线
                ├── V2__insert_categories.sql # 📋 计划中
                └── V3__add_fulltext_index.sql # 📋 计划中
```

> **实现进度回写（v1.6，2026-06-16）**：阶段 0–5 已完成，上表"📋 源码待实现"标记按实际进度更新如下：
>
> | 模块 | 状态 | 说明 |
> |------|------|------|
> | `library-common` | ✅ 已实现 | 异常体系 / Result / PageResult / 公共 DTO / 工具类 / `@NoAuth` |
> | `library-security` | ✅ 已实现 | JWT 过滤器 / RBAC 切面 / 限流 / Spring Security 配置 / **认证与业务 Controller** |
> | `library-core` | ✅ 已实现 | 图书检索 / 借阅 / 预约 Service + Mapper + Entity + ES 同步 / 图书与分类 Controller |
> | `library-bootstrap` | ✅ 已实现 | 启动类 / CORS / Jackson / MyBatis-Plus / Async / Scheduling 配置 / Flyway V1–V4 |
> | `library-ai` | ✅ 已实现 | 阶段 5（LLM / Embedding / NLP — DeepSeek + 阿里云百炼 + HanLP），28 项测试全绿 |
> | `library-knowledge-graph` | 📋 待实现 | 阶段 7（Neo4j 图谱） |
> | `library-acquisition` | 📋 待实现 | 阶段 8（智能采编） |
>
> **Controller 落位决策（ADR）**：认证与需鉴权上下文的业务 Controller（`AuthController`、`BorrowController`、`ReservationController`、`UserCenterController`、`AdminBookController`、`AdminBorrowController`）置于 `library-security` 模块，而非本节结构图所示的 `library-core`。原因：这些 Controller 强依赖 `SecurityUtils`（security 域）获取当前登录用户，若放 core 将形成 `security → core → security` 循环依赖。无需鉴权上下文的 Controller（`BookController`、`CategoryController`）仍置 `library-core`。Service 与 Mapper 一律置 `library-core`，Controller 仅做参数编排与调用 Service。
...
└── db/
    └── migration/                      # Flyway 数据库迁移脚本
        ...
        ├── V2__insert_categories.sql   # 📋 计划中（随核心模块实现补全）
        └── V3__add_fulltext_index.sql  # 📋 计划中
```
